...
export interface NJDistrictData {
  district: string;
...
export default function NJDistrictChoroplethMap({
  data,
  selectedDistrict,
  onSelect,
}: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const njFeatures = json.features.filter(
          (f) => f.properties.STATEFP === NJ_STATE_FIPS,
        );
        if (njFeatures.length === 0) {
          throw new Error('No New Jersey features in the geojson');
        }
        const bbox = computeBBox(njFeatures);
        const project = makeProjector(bbox);
        const geom: DistrictGeometry[] = njFeatures.map((f) => {
  ...
  const dataByDistrict = useMemo(() => {
    const map = new Map<string, NJDistrictData>();
    data.forEach((d) =>
    ...
    <div className="relative">
      {geoError && (
        <div className="mb-3 rounded-md border border-yellow-200 bg-yellow-50 px-3 py-2 text-sm text-yellow-800">
          Could not load the NJ geography ({geoError}).
        </div>
      )}
      <svg
        ...
        role="img"
        aria-label="Choropleth of New Jersey's 12 congressional districts"
      >
        {geometries &&
          geometries.map((g) => {
      ...
      {tooltip && tooltipData && (
        ...
        >
          <p className="font-semibold text-gray-900">
            NJ-{String(tooltipData.district_number).padStart(2, '0')}
          </p>
